...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    /**
     * Download media from an inbound message and return it as inline base64.
     *
     * Deliberately does not touch the filesystem. The adapter runs before
     * routing has picked a session, so it cannot know which session directory
     * the file belongs in — writing it anywhere else puts it outside the
     * `/workspace` mount and the agent simply cannot see it.
     *
     * `extractAttachmentFiles` in session-manager.ts is the one place that
     * knows the session, and it only acts on attachments carrying inline
     * `data`. It writes them to `inbox/<messageId>/<name>` through
     * inbox-safety's symlink guards, then rewrites `localPath` to the
     * container-visible path. Handing it bytes is what puts the file at
     * `/workspace/inbox/<messageId>/<name>`.
     */
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async function downloadInboundMedia(
      ...
      for (const { key, type, ext } of mediaTypes) {
        ...
        try {
          ...
          // WhatsApp's E2E channel — Meta can't sanitize it server-side. Kept
          // here as well as in extractAttachmentFiles: this is the earliest
          // point the name is known, and the log line names the adapter that
          // saw it. The write-side guard stays the load-bearing one.
          ...
          if (rawFilename && filename !== rawFilename) {
            log.warn('Refused unsafe attachment filename — would escape the inbox', {
              rawFilename,
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // documentMessage.caption is the text typed alongside a file. It
            // arrives wrapped as `documentWithCaptionMessage`, which
            // normalizeMessageContent unwraps to a plain documentMessage — so
            // it reads like the image/video cases despite the different wire
            // shape. Without it, "summarise this" sent with a .docx reaches the
            // agent as an attachment and an empty string, and the agent has no
            // choice but to ask what the file is for.
            let content =
            ...
            //
            // Deliberately ahead of the media download: WhatsApp echoes every
            // file the bot sends straight back, and downloading it only to drop
            // it here costs a full CDN fetch and a base64 copy of the payload
            // in memory. Nothing below this point needs the echo.
            if (fromMe) {
    ...
    const adapter: ChannelAdapter = {
      ...
      async deliver(
        ...
        if (hasFiles) {
          ...
          // The id of the first file that actually reached WhatsApp. deliver()
          // returns the platform message id the host records as
          // platform_message_id; returning nothing here logged every attachment
          // as `platformMsgId=undefined`, which is the exact signature of a
          // message marked delivered but never sent — so a real send was
          // indistinguishable from the silent-loss failure mode.
          ...
          // Caption consumed the text, or there was none to send: this message
          // is fully delivered as files. `!text` matters as much as
          // `captionUsed` — a file-only message previously fell through to the
          // text branch, sent nothing, and also reported no id.
          if (captionUsed || !text) return firstSentId;
        }
